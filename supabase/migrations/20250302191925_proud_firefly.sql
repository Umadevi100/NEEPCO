/*
  # Enable vendor payment processing

  1. Updates
    - Add policy for vendors to update their own payments
    - Add function to validate vendor payment updates
    - Add trigger to handle payment status changes
    
  2. Security
    - Only allow vendors to update their own pending payments
    - Only allow status change to 'processing'
    - Require transaction ID and payment method
*/
  ...
  IF NEW.status = 'processing' AND OLD.status = 'pending' THEN
    ...
    LOOP
      INSERT INTO notifications (
        ...
        'payment_submitted',
        'New payment submitted by ' || vendor_name || ' requires verification',
        NEW.id
      );
    END LOOP;
  END IF;
